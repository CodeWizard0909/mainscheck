...
├── docs/adr/           ← does not exist yet; created lazily
├── mainscheck/         ← the package
└── tickets/
```

Multi-context layout (a `CONTEXT-MAP.md` at the root pointing to per-context
`CONTEXT.md` files) does not apply here and should not be introduced.
